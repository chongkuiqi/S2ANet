...
def prepare_multi_scale_data(src_path, dst_path, gap=200, subsize=1024, scales=[0.5, 1.0, 1.5], num_process=32):
    ...
    split_test = splitbase_test(osp.join(src_path, 'val'), dst_test_path,
                                gap=gap, subsize=subsize, num_process=num_process)
    for scale in scales:
...
if __name__ == '__main__':
    ...
    #                          num_process=32)
    prepare_multi_scale_data('/data/hjm/dota', '/data/hjm/dota_1024', gap=320, subsize=1024, scales=[1.0],
                             num_process=32)
