...
class ResNet(nn.Module):
    ...
    def __init__(self, model_name, num_classes=1000, use_SE=False):
        ...
        self.layer1 = self._make_layer(block, 64, layers_cfg[0], use_SE=use_SE)

        # 第3次下采样
        self.layer2 = self._make_layer(block, 128, layers_cfg[1], stride=2, use_SE=use_SE)
        self.layer3 = self._make_layer(block, 256, layers_cfg[2], stride=2, use_SE=use_SE)
        self.layer4 = self._make_layer(block, 512, layers_cfg[3], stride=2, use_SE=use_SE)
    ...
    def _make_layer(self, block, plane, num_blocks, stride=1, use_SE=False):
        '''
        ...
        layers.append(block(self.inplanes, plane, stride, downsample, use_SE=use_SE))
        
        # 当前的特征图的通道数发生变化
        self.inplanes = plane * block.expansion
        # 该layer后续的残差块，输入特征图与输出特征图不需要调整尺寸和通道数
        for i in range(1, num_blocks):
            layers.append(block(self.inplanes, plane, use_SE=use_SE))
...
class DetectorBackbone(nn.Module):
    def __init__(self, backbone_name, frozen_stages=1, norm_eval=False, out_indices=(2,3,4)):
        ...
        # 创建ResNet分类网络模型，并使用SE模块
        clasify_model = ResNet(self.backbone_name, use_SE=True)
        ...
        # 使用SE官方提供的在ImageNet上的预训练模型
        state_dict = torch.load("/home/lab/ckq/S2ANet/offical_resnet50_SE.pkl", map_location=torch.device("cpu"))
