...
from utils.loss import SmoothL1Loss, FocalLoss, mmFocalLoss, GDLoss_v1, LARLoss
...
class S2ANetHead(nn.Module):
    ...
    def __init__(self, num_classes, in_channels=256, feat_channels=256, stacked_convs=2, 
        ...
        # 回归损失是否使用KLD
        reg_kld = False,
        # 回归损失使用KLD的情况下，回归损失的计算必须使用解码后的边界框，而不能使用解码后的边界框
    ...
    def _init_loss_func(self):
        ...
        loss_conf = FocalLoss(loss_conf, self.odm_fl_gamma, self.odm_fl_alpha)
    ...
    def _init_layers(self):
        ...
        ## 添加一个置信度检测头，类似于YOLO的置信度分支，这个头使用和reg分支相同的卷积层
        self.conf_head = nn.Conv2d(self.feat_channels, 1, kernel_size=(3,3), padding=1, bias=True)
    ...
    def create_unshared_head_layers(self):
        if not self.shared_head:
            ...
            self.conf_head = nn.ModuleList(self.conf_head for i in range(self.num_levels))
    ...
    def forward_single(self, x, featmap_stride):
        ...
        tmp = self.odm_reg_ls(or_feat)
        odm_bbox_pred = self.odm_reg_head(tmp)
        conf_pred = self.conf_head(tmp)
        # shape [B,1，H, W] to shape [B, H, W, 1]
        ...
        return fam_cls_pred, fam_bbox_pred, odm_cls_pred, odm_bbox_pred, init_grid_anchors, refine_anchor, conf_pred
    ...
    def forward_single_unshared_head(self, x, featmap_stride):
        ...
        tmp = self.odm_reg_ls[level_id](or_feat)
        odm_bbox_pred = self.odm_reg_head[level_id](tmp)
        conf_pred = self.conf_head[level_id](tmp)
        # shape [B,1，H, W] to shape [B, H, W, 1]
        conf_pred = conf_pred.permute(0, 2, 3, 1).contiguous().reshape(batch_size, feat_h, feat_w, -1)
    ...
    def compute_loss(self, p, targets):
        ...
        for level_id in range(num_level):
            ...
            conf_pred = p[6][level_id].reshape(-1)
        ...
        total_loss = fam_cls_loss + fam_reg_loss + odm_cls_loss + odm_reg_loss + conf_loss
        return total_loss, \
            torch.cat((fam_cls_loss,fam_reg_loss,odm_cls_loss,odm_reg_loss, conf_loss)).detach().cpu().numpy()
    ...
    def compute_loss_single_level(self, bbox_pred, cls_pred, anchors, pos_targets_batch, assign_gt_ids_batch, module_name, conf_pred=None, max_ious_levels=None):
        ...
        conf_loss = torch.zeros(1,device=device)
        if max_ious_levels is not None:
    ...
    def get_bboxes(self, p, module_name="odm"):
        ...
        for batch_id in range(batch_size):
            ...
            for level_id in range(num_level):
                ...
                conf_one_img_one_level = conf_pred[level_id][batch_id].detach().reshape(-1, 1)
                ...
                conf_levles.append(conf_one_img_one_level)

            # 进行一张图像的NMS
            det_bboxes, det_labels = self.get_bboxes_single_img(scores_levels, bbox_pred_levels, anchors_levels, conf_levles)

            imgs_results_ls.append((det_bboxes, det_labels))
        
        return imgs_results_ls

    def get_bboxes_single_img(self, scores_levels, bbox_pred_levels, anchors_levels, conf_levels):
        ...
        anchors = []
        # 逐个特征层级进行处理
        for score_level, bbox_pred_level, anchors_level, conf_level in zip(scores_levels, bbox_pred_levels, anchors_levels,conf_levels):
            
            score_level = score_level.sigmoid() * conf_level.sigmoid()
            # score_level = conf_level.sigmoid()
