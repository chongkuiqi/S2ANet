...
std::tuple<at::Tensor, at::Tensor> RIE_forward_cuda(const at::Tensor& feature,
  ...
  AT_DISPATCH_FLOATING_TYPES(feature.scalar_type(), "RIE_forward", [&] {
    RIE_forward_cuda_kernel<scalar_t><<<grid, block, 0, stream>>>(
         count,
         feature.contiguous().data_ptr<scalar_t>(),
         nBatch,
         nFeature,
         nOrientation,
         mainDirection.contiguous().data_ptr<uint8_t>(),
         aligned.contiguous().data_ptr<scalar_t>());
  });
  THCudaCheck(cudaGetLastError());
  return std::make_tuple(mainDirection, aligned);
}
...
at::Tensor RIE_backward_cuda(const at::Tensor& mainDirection,
  ...
  AT_DISPATCH_FLOATING_TYPES(gradOutput.scalar_type(), "RIE_backward", [&] {
    RIE_backward_cuda_kernel<scalar_t><<<grid, block, 0, stream>>>(
         count,
         gradOutput.contiguous().data_ptr<scalar_t>(),
         mainDirection.contiguous().data_ptr<uint8_t>(),
         nBatch,
         nFeature,
         nOrientation,
         gradInput.contiguous().data_ptr<scalar_t>());
  });
  THCudaCheck(cudaGetLastError());
  return gradInput;
}