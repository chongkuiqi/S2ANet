...
at::Tensor ARF_forward_cuda(const at::Tensor& weight,
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(weight.scalar_type(), "ARF_forward", [&] {
    ARF_forward_cuda_kernel<scalar_t><<<grid, block, 0, stream>>>(
         output_size,
         weight.contiguous().data_ptr<scalar_t>(),
         indices.contiguous().data_ptr<uint8_t>(),
         nInputPlane,
         nOutputPlane,
         nOrientation,
         nRotation,
         nEntry,
         output.contiguous().data_ptr<scalar_t>());
  });
  THCudaCheck(cudaGetLastError());
  return output;
}
...
at::Tensor ARF_backward_cuda(const at::Tensor& indices,
  ...
  AT_DISPATCH_FLOATING_TYPES(gradOutput.scalar_type(), "ARF_backward", [&] {
    ARF_backward_cuda_kernel<scalar_t><<<grid, block, 0, stream>>>(
         ...
         nRotation,
         nEntry,
         gradWeight.contiguous().data_ptr<scalar_t>());
  });
  THCudaCheck(cudaGetLastError());
  return gradWeight;
}